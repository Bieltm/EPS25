...
# --- 3. CARGA Y ALGORITMO (AHORA CON ENTEROS 1-10) ---
@st.cache_data
def load_data_with_api():
    ...
    # --- ALGORITMO ENTERO (1-10 SIN DECIMALES) ---
    def calculate_smart_score(row, column, target_ratio_per_10k, min_critical_mass):
        ...
        if row[column] < min_critical_mass:
            ...
            raw_score = penalty_factor * 4.0 
        else:
            ratio = (row[column] / row['poblacion']) * 10000
            raw_score = (ratio / target_ratio_per_10k) * 10
            
        # 2. REDONDEO Y ENTEROS
        # round() redondea al más cercano (7.6 -> 8)
        # int() quita el punto decimal
        # max(1, ...) y min(10, ...) aseguran el rango
        return int(max(1, min(10, round(raw_score))))
...
st.caption(f"Algoritmo v2.3: Scores enteros (1-10). Perfil seleccionado: {perfil}")
...
with st.expander("🛠️ Ver Datos (Enteros 1-10)"):
    st.dataframe(df_sorted[['barrio', 'match_percentage', 'vida_nocturna', 'naturaleza_score', 'movilidad_score']])